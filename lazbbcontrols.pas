...
type
  ...
  // Compatibility with old component name

  TSCrollLabel = TLabelScroll;
...
begin
   ...
   //RegisterComponents('lazbbComponents',[TLabelSCroll]);
...
// TLabelScroll procedures
constructor TLabelScroll.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  Parent:= TWinControl(aOwner);
  FCaption:='ScrollButton';
...
procedure TLabelScroll.OnTimerCanvas(sender: TObject);
begin
...
procedure TLabelScroll.SetCaption(AValue: string);
begin
  if fCaption=AValue then exit;
  FCaption:=AValue;
  inherited Caption:=FCaption;
...
procedure TLabelScroll.SetMargin(AValue: integer);
begin
  if FMargin=AValue then exit;
  FMargin:=AValue;
  //inherited Margin:= AValue;
...
procedure TLabelScroll.SetSCrolling(AValue: Boolean);
begin
  if FScrolling= AValue then exit;
  FSCrolling:= AValue;
  ReInit;
...
procedure TLabelScroll.SetScrollInterval(AValue:integer);
begin
  if FScrollInterval= AValue then exit;
  FScrollInterval:= AValue;
  FTimerScroll.Interval:= AValue;
...
procedure TLabelScroll.SetSCrollAutoString(AValue:string);
begin
  if FSCrollAutoString= AValue then exit;
  FSCrollAutoString:= AValue;
  ReInit;
...
procedure TLabelScroll.SetSCrollGraph(AValue:Boolean);
begin
  if fScrollGraph= AValue then exit;
  fsCrollGraph:= AValue;
  Reinit;
...
procedure TLabelScroll.SetSCrollStep(AValue:Integer);
begin
  if fScrollStep=aValue then exit;
  fSCrollStep:= AValue;
  ReInit;
...
procedure TLabelScroll.SetSCrollDirection(aValue: TSCrollDirection);
begin
  if FScrollDirection=aValue then exit;
  FScrollDirection:= aValue;
  if aValue=sdLeftToRight then FTimerScroll.OnTimer:= @OnTimerScrollL
...
destructor TLabelScroll.Destroy;
begin
...
procedure TLabelScroll.OnTimerScrollL(Sender: TObject);
begin
...
procedure TLabelScroll.OnTimerScrollR(Sender: TObject);
begin
  if fSCrollGraph then
  begin
    if (ScrollIndex=0) then ReInit;
